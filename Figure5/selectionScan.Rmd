...
## Comparison with a list  loci/gene

```{r}
interesting.loci <- read.table("OFcandidates.txt", header = TRUE) %>% 
  mutate(Location = paste0(Chromosome,":",Positions))
interesting.gene <- merge(interesting.loci, vep.res, by = c("Location"))
```
...
id.max <- 100
tess3.cand <- sort(tess3.scan$q.value, index.return = TRUE)$ix[1:id.max]
prop <- mean(interesting.loci$Index %in% tess3.cand)
print(paste0("prop of interesting loci in tess3 cand list  = ",prop))

# top 100 gene
id.max <- 100
tess3.cand <- unique(tess3.scan.annotation$Gene)[1:id.max]
prop <- mean(unique(interesting.gene$Gene) %in% tess3.cand)
print(paste0("prop of interesting gene in tess3 cand list  = ",prop))
```
